...
[box login]		
	<fieldset id='login'>
		...
		{.if| {.length|%user%.} |{:
            %user%
            {.if|{.can change pwd.} | 
                <button onclick='changePwd.call(this)' style='font-size:x-small;'>{.!改变密码.}</button>
            .}
            :}
            | <a href="~login">登陆</a>
        .}
...
[box folder]
	<fieldset id='folder'>
       ...
       <div style='float:right; position:relative; top:-1em; font-weight:bold;'>
		...
		| {.if| {.%folder% != / .} | <a href=".."><img src="/~img14"> {.!向上.}</a> .}
		/if.}
		</div>

		<div id='breadcrumbs'>
		{.comment|we need this to start from 1 {.count|folder levels.}.}
		{.breadcrumbs|{:<a href="%bread-url%" {.if|{.length|%bread-name%.}|style='padding-left:{.calc|{.count|folder levels.}*0.7.}em;'.} /> {.if|{.length|%bread-name%.}|&raquo; %bread-name%|<img src="/~img1"> {.!主页.}.}</a>:} .}
       </div>
        
		<div id='folder-stats'>%number-folders% {.!文件夹.}, %number-files% {.!文件.}, {.add bytes|%total-size%.}
		</div>
...
[box search]	
	...
	<fieldset id='search'>
		...
		<div style='margin-top:0.5em;' class='hidden popup'>
			<fieldset>
				<legend>{.!搜索哪里.}</legend>
					<input type='radio' name='where' value='fromhere' checked='true' />  {.!这个文件夹和子文件夹.}
					<br><input type='radio' name='where' value='here' />  {.!这个文件夹.}
...
[box selection]	
	<fieldset id='select' class='onlyscript'>
    	...
    	<button onclick="
			...
			selectedChanged();
            ">{.!反选.}</button>
    	<button onclick='selectionMask.call(this)'>{.!标记.}</button>
		<p style='display:none; margin-top:1em;'><span id='selected-number'>0</span> 项目 选中</p>
		</center>
	</fieldset>
...
[box actions]	
	<fieldset id='actions'>
		...
		<button id='renameBtn' onclick='
            var a = selectedItems();
                if (a.size() != 1)
				return alert("你必须选择唯一的一个项目来重命名");
			ezprompt(this.innerHTML, {type:"text"}, function(s){
				ajax("rename", {from:getItemName(a[0]), to:s});
		    });'>{.!重命名.}</button>
		.}
		{.if|{.get|can archive.}|
		<button id='archiveBtn' onclick='if (confirm("{.!confirm.}")) submit({}, "{.get|url|mode=archive|recursive.}")'>{.!打包.}</button>
		.}
...
[box server info]
	<fieldset id='serverinfo'>
		...
		<a href="http://ano.f3322.net/">Http文件服务器 %version%</a>
		<br />{.!服务器时间.}: %timestamp%
		<br />{.!服务器运行时间.}: %uptime%
	</fieldset>
...
[+special:strings]
max s dl msg=这台服务器上有 <b>simultaneous</b> 下载限制.<br>现在已经达到了限制. 请稍后再试.
retry later=请稍后再试.
item folder=进入文件夹
no files=文件夹里没有文件
no results=没有项目匹配你的查询请求
confirm=你确定吗?
...
[file=folder=link|private]
	<tr class='{.if|{.mod|{.count|row.}|2.}|even.}'><td>
		...
		{.if| {.length|{.?search.}.} |{:{.123 if 2|<div class='item-folder'>{.!项目 文件夹.} |{.breadcrumbs|{:<a href="%bread-url%">%bread-name%/</a>:}|from={.count substring|/|%folder%.}/breadcrumbs.}|</div>.}:} .}
...
[+folder]
<td class='nosize'>文件夹<td>%item-modified%<td>%item-dl-count%

[+link]
<td class='nosize'>链接<td colspan='2'>
...
$(function(){
    ...
    if  (x.size()) {
        ...
        x.find('form').submit(function(){ 
            ...
            // build the gui
            x.append("<div id='progress'>正在处理...</div>");
            x.append($("<button style='float:right'>{.!取消.}</button>").click(function(){
                // stop submit/upload
